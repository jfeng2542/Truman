...
exports.getScriptFeed = (req, res, next) => { 
  ...
  //{
    ...
    Script.find()
      ...
      //.where(scriptFilter).equals("yes")        // **** have it do a text search. Create a text index (see link with documentation)
      //.where('time').lte(0)
      .sort('-time')
      .populate('actor')
      .populate({ 
...
};//end of .getScriptFeed



/**
 * GET /
 * Queried list of Script posts for Feed
 * Made for testing
*/
exports.getScriptFeedSearch = (req, res, next) => { 
  ...
  //{
    ...
    Script.find( { $text: { $search: "\"" + req.query.search.toString() + "\""} } )
      //change this if you want to test other parts
      //.where(scriptFilter).equals("yes")        // **** have it do a text search. Create a text index (see link with documentation)
      //.where('time').lte(0)
      .sort('-time')
...
exports.newPost = (req, res) => {
  ...
  User.findById(req.user.id, (err, user) => {
   ...
   // console.log(lastFive +" just called to create a new post");
    ...
    //Checks for username tags and hashtags to substitute them with links
    //Eg：Hello my friend @brian #funtimes
